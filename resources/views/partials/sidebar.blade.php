...
>
    ...
    <header class="fixed left-0 right-0 top-0 z-50 border-b border-teal-950/50 bg-gradient-to-r from-teal-950 via-teal-900 to-stone-950 pt-[env(safe-area-inset-top,0px)] shadow-lg shadow-teal-950/40 backdrop-blur-md lg:hidden">
        <div class="flex h-14 items-center gap-2 px-3">
            ...
            <a href="{{ route('dashboard') }}" class="flex min-w-0 flex-1 items-center justify-center gap-2.5">
                @include('partials.brand-mark', ['class' => 'h-9 w-9'])
                <span class="truncate font-display text-base font-semibold tracking-wide text-amber-50">{{ config('app.name') }}</span>
            </a>
    ...
    >
        <div class="flex flex-1 flex-col overflow-y-auto overscroll-contain p-5 pt-6 lg:p-6">
            <a href="{{ route('dashboard') }}" class="mb-8 hidden items-center gap-3 rounded-xl p-2 -m-2 transition hover:bg-white/5 lg:flex" @click="sidebarOpen = false">
                @include('partials.brand-mark', ['class' => 'h-12 w-12'])
                <div class="min-w-0">
                    <span class="font-display text-lg font-semibold tracking-wide text-amber-100">{{ config('app.name') }}</span>
            ...
            <div class="mb-6 border-b border-teal-800/80 pb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        <p class="truncate font-medium text-stone-50">{{ Auth::user()->name }}</p>
                        <p class="truncate text-sm text-teal-200/80">{{ Auth::user()->email }}</p>
                    </div>
                </div>
            </div>
            ...
            <nav class="space-y-1">
                ...
                <a href="{{ route('dashboard') }}" class="{{ $d['classes'] }}" @click="sidebarOpen = false">
                    <svg class="h-5 w-5 shrink-0 opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $d['icon'] }}"></path></svg>
                    <span>{{ $d['label'] }}</span>
                </a>
                ...
                <a href="{{ route('pdf.index') }}" class="{{ $i['classes'] }}" @click="sidebarOpen = false">
                    <svg class="h-5 w-5 shrink-0 opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $i['icon'] }}"></path></svg>
                    <span>{{ $i['label'] }}</span>
                </a>
                ...
                <a href="{{ route('pdf.merge.create') }}" class="{{ $m['classes'] }}" @click="sidebarOpen = false">
                    <svg class="h-5 w-5 shrink-0 opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $m['icon'] }}"></path></svg>
                    <span>{{ $m['label'] }}</span>
                </a>
                ...
                <a href="{{ route('pdf.compress.create') }}" class="{{ $c['classes'] }}" @click="sidebarOpen = false">
                    <svg class="h-5 w-5 shrink-0 opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $c['icon'] }}"></path></svg>
                    <span>{{ $c['label'] }}</span>
                </a>
                ...
                <a href="{{ route('pdf.convert.create') }}" class="{{ $cv['classes'] }}" @click="sidebarOpen = false">
                    <svg class="h-5 w-5 shrink-0 opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $cv['icon'] }}"></path></svg>
                    <span>{{ $cv['label'] }}</span>
                </a>
            </nav>
        </div>
